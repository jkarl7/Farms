...
class FilterJointFarmsByRulesTest {
    ...
    @DisplayName("GIVEN two connected farms where one has 0.29 hectares area and other 0.01 WHEN area filter is applied " +
    ...
    @DisplayName("GIVEN two connected farms (one has area 0.30 and other 0.30) hectares area WHEN area filter is applied " +
            "THEN this joint area should be invalid because we don't display connected farms where all have >= 0.3")
    @Test
    void validJointAreaTest2() {
        ...
        item2.setFarmNo("55-1");
        item2.setFieldNo(1L);
        item2.setArea(new BigDecimal("0.3"));
        item2.setIsActive(true);
